...
namespace gcgcg
{
  internal class Objeto  //TODO: deveria ser uma class abstract ..??
  {
    ...
    public void RemoverPontoMaisPertoMouse(Ponto4D mouseLocation)
    {
      if (pontosLista.Count == 1) {
        return;
      }
      pontosLista.RemoveAt(PontoMaisPerto(mouseLocation));
      ObjetoAtualizar();
    }

    public void RemoverObjeto(Objeto objetoSelecionado)
    {
    ...
    public int PontoMaisPerto(Ponto4D mouseLocation) {
        int indice = -1;
        double dist = double.MaxValue;
      
        for (int i = 0; i < pontosLista.Count; i++) {
          double distanciaPonto = Matematica.Distancia(mouseLocation, pontosLista[i]);

          if (distanciaPonto < dist) {
            dist = distanciaPonto;
            indice = i;
          }
        }
